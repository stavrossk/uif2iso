...
int main(int argc, char *argv[]) 
{
    ...
    if(fseek(fdi, file_size - sizeof(bbis), SEEK_SET)) 
    {
        ...
        if( ((file_size - sizeof(bbis)) > 0x7fffffff) && ((file_size - sizeof(bbis)) < file_size) ) 
            printf("  an error here means that your exe has no full LARGE_FILES 64 bit support!\n");        std_err();
    ...
    } else if(bbis.image_type == 9) 
    {
        ...
        } else 
        {
            ...
            if(fseek(fdi, 4, SEEK_CUR)) 
                std_err();  // additional data to skip
    ...
    for(i = 0; i < blhr.num; i++) 
    {
        ...
        if(fseek(fdo, (u64)blhr_data[i].sector * (u64)bbis.sectorsz, SEEK_SET)) 
            std_err();
...
void magiciso_is_invalid(FILE *fd, u64 nrgoff, u8 *fileo) 
{
    ...
    {   // get tracks and do more things
        ...
        if(!memcmp(chunk.id, "SINF", 4)) 
        {  // usually located after DAO
        
            if(chunk.size >= 4) 
            {
                
                if(fseek(fd, 3, SEEK_CUR)) 
                    break;
...
void uif_crypt_key(u8 *key, u8 *pwd) 
{
    ...
    for(i = 1; i < 4; i++) 
    {
        ...
        if(!endian) 
        {   // this solution is required for little/big endian compatibility and speed
            k[0] += k[i];
            continue;
        }
